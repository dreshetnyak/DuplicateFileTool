...
internal sealed class ResultsGroupInclusionPredicate : IInclusionPredicate<DuplicateGroup>
{
    ...
    public bool IsIncluded(DuplicateFile duplicateFile)
    {
        ...
        {
            if (Filter.IsFilterFilePath && path != null && path.Contains(keyword, stringComparison))
                return matchResult;
            if (Filter.IsFilterFileName && name != null && name.Contains(keyword, stringComparison))
                return matchResult;
            if (Filter.IsFilterFileExtension && extension != null && extension.Contains(keyword, stringComparison))
                return matchResult;
        }
