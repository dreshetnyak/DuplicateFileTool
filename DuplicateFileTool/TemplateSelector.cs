...
internal sealed class ParameterTemplate(string enabled, string disabled)
{
...
internal abstract class TemplateSelectorBase : DataTemplateSelector
{
    ...
    public override DataTemplate? SelectTemplate(object? item, DependencyObject container)
    {
        ...
        if (!ParameterTemplates.ContainsKey(valueType))
            return (DataTemplate)element.FindResource(DefaultTemplateName);
        return (DataTemplate)element.FindResource(isReadOnly 
            ? ParameterTemplates[valueType].Disabled 
            : ParameterTemplates[valueType].Enabled);
    }
}

internal sealed class ConfigTemplateSelector : TemplateSelectorBase
{
...
internal sealed class ConfigEditTemplateSelector : TemplateSelectorBase
{
